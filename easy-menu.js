...
function startSession(order, mode, label) {
  ...
  window.location.href = `./easy-scenarios/${safeOrder[0]}/`;
}
...
function renderScenarioGrid() {
  ...
  scenarioGrid.innerHTML = scenarios
    .map(
      (scenario) => `
        <a class="scenario-link" href="./easy-scenarios/${scenario.slug}/">
          <span class="scenario-number">${String(scenario.number).padStart(2, "0")}</span>
          <p class="scenario-title">${scenario.title}</p>
          <p class="scenario-meta">Open this easy-mode scenario</p>
        </a>
      `
    )
    .join("");
  ...
  if (activeSession?.active) {
    ...
    if (resumeBtn) {
      ...
      resumeBtn.onclick = () => {
        ...
        window.location.href = `./easy-scenarios/${slug}/`;
      };
    }
