const scenarios = [window.THREE_MAN_SCENARIO || { paths: [] }];
...
const drawU3Btn = document.getElementById("draw-u3-btn");
...
const DEFAULT_SCENARIO_ORDER = THREE_MAN_APP.scenarios.length
  ? THREE_MAN_APP.scenarios.map((scenario) => scenario.slug)
  : ["01-fly-ball-u1-has-catch-no-catch"];
const currentScenarioSlug = window.location.pathname.replace(/\/$/, "").split("/").pop();
const scenarioMetaBySlug = Object.fromEntries(
  (THREE_MAN_APP.scenarios || []).map((scenario) => [
    scenario.slug,
...
function getScenarioMaxPoints(slug) {
  return scenarioMetaBySlug[slug]?.points || scenarios[0].paths.length;
}
...
function updateNextButtonLabel() {
  const session = getPlayAllSession();
  if (!session) {
    nextBtn.textContent = DEFAULT_SCENARIO_ORDER.length > 1
      ? "Next Situation"
      : "Back to 3-Man Situations";
    return;
  }
...
function goToNextPlayAllScenario() {
  ...
  window.location.href = "../../3man-situations.html";
  return true;
}
...
function initializeRunnerLayer() {
  runnerLayer = document.createElementNS(SVG_NS, "g");
  runnerLayer.setAttribute("id", "runner-overlay");
  runnerLayer.setAttribute("class", "runner-overlay");
  field.insertBefore(runnerLayer, ballFlight);
}

function renderRunnerMarkers(runners = []) {
  if (!runnerLayer) {
    return;
  }
  ...
  runners.forEach((runner) => {
    ...
    marker.setAttribute("cx", runner.x);
    marker.setAttribute("cy", runner.y);
    marker.setAttribute("r", 16);
    marker.setAttribute("class", "runner-marker");
    runnerLayer.appendChild(marker);

    const label = document.createElementNS(SVG_NS, "text");
    label.setAttribute("x", runner.x);
    label.setAttribute("y", runner.y + 0.5);
    label.setAttribute("class", "runner-label");
    label.textContent = runner.label;
    runnerLayer.appendChild(label);
  });
}
...
function renderAnswerPath(container, pathConfig, roleClass) {
  container.innerHTML = "";

  const appendStationaryMarker = (points) => {
    const stationaryPoint = points[points.length - 1] || points[0];
    if (!stationaryPoint) {
      return;
    }

    const marker = document.createElementNS(SVG_NS, "circle");
    marker.setAttribute("cx", stationaryPoint.x);
    marker.setAttribute("cy", stationaryPoint.y);
    marker.setAttribute("r", 7);
    marker.setAttribute("class", `answer-path-stationary${roleClass}`);
    marker.setAttribute(
      "style",
      "fill: #ffcf5c; stroke: rgba(255, 255, 255, 0.96); stroke-width: 3;"
    );
    container.appendChild(marker);
  };

  const renderSegment = (points, dashed = false) => {
    if (!Array.isArray(points) || points.length < 2) {
      return;
    }

    const hasVisibleMovement = points.some(
      (point, index) => index > 0 && distance(point, points[index - 1]) >= 6
    );

    if (!hasVisibleMovement) {
      appendStationaryMarker(points);
      return;
    }

    const segmentLine = document.createElementNS(SVG_NS, "polyline");
    segmentLine.setAttribute(
      "points",
      points.map((point) => `${point.x},${point.y}`).join(" ")
    );
    segmentLine.setAttribute(
      "class",
      `answer-path-segment${roleClass}${dashed ? " is-dashed" : ""}`
    );
    container.appendChild(segmentLine);
  };

  if (Array.isArray(pathConfig.answerSegments) && pathConfig.answerSegments.length) {
    pathConfig.answerSegments.forEach((segment) => {
      renderSegment(segment.points, Boolean(segment.dashed));
    });
    container.classList.remove("hidden");
    return;
  }

  renderSegment(pathConfig.routePoints || [pathConfig.startAnswer, pathConfig.endAnswer]);
  container.classList.remove("hidden");
}
...
function getDisplayedMaxPoints() {
  ...
  if (session?.active && Array.isArray(session.order) && session.order.length) {
    return session.order.reduce(
      (sum, slug) => sum + getScenarioMaxPoints(slug),
      0
    );
  }
...
function roleDisplayName(role) {
  ...
  if (role === "U1") return "First Base Umpire";
  if (role === "U3") return "Third Base Umpire";
  return role;
}
...
function updatePrompt() {
  ...
  if (activeRoleIndex === null) {
    feedbackTitleEl.textContent = "Choose P, U1, or U3 to begin drawing.";
...
function updateRoleDrawButtons() {
  const scenario = scenarios[scenarioIndex];
  const buttonMap = { P: drawPBtn, U1: drawU1Btn, U3: drawU3Btn };
...
function updateCursorReadout(point) {
  ...
  cursorReadoutEl.classList.add("hidden");
}
...
function finishActiveRolePath() {
  ...
  if (activeRoleIndex === null) {
    feedbackTitleEl.textContent = "Check all three movement paths when you're ready.";
    renderMovementTracker();
  } else {
    updatePrompt();
  }
...
function loadScenario() {
  ...
  renderRunnerMarkers(scenario.runnerMarkers || []);
...
[
  { button: drawPBtn, index: 0 },
  { button: drawU1Btn, index: 1 },
  { button: drawU3Btn, index: 2 },
...
checkBtn.addEventListener("click", () => {
  ...
  scenario.paths.forEach((path, index) => {
    ...
    const artifacts = roleArtifacts[index];
    const roleClass = path.role ? ` role-${path.role.toLowerCase()}` : "";
    placeMarker(artifacts.answerStart, path.startAnswer);
    placeMarker(artifacts.answerEnd, path.endAnswer);
    renderAnswerPath(artifacts.answerPath, path, roleClass);
  });
...
nextBtn.addEventListener("click", () => {
  ...
  const hasStandaloneNextScenario =
    currentIndex !== -1 && currentIndex < DEFAULT_SCENARIO_ORDER.length - 1;
  const nextSlug = hasStandaloneNextScenario
    ? DEFAULT_SCENARIO_ORDER[currentIndex + 1]
    : null;
