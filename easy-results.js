...
function renderResults() {
  ...
  replayBtn.textContent =
    result.mode === "random-5" ? "Play Another Easy Random 5" : "Replay Easy Mode";
}
...
replayBtn.addEventListener("click", () => {
  ...
  const session = {
    ...
    label: isRandomFive ? "Easy Mode Random 5" : "Easy Mode All Scenarios",
  };

  localStorage.removeItem(PLAY_ALL_COMPLETE_KEY);
  setStoredJson(PLAY_ALL_KEY, session);
  window.location.href = `./easy-scenarios/${order[0]}/`;
});
