...
function renderResults() {
  ...
  replayBtn.textContent =
    result.mode === "random-5" ? "Play Another Random 5" : "Replay All Situations";
}
...
replayBtn.addEventListener("click", () => {
  ...
  const session = {
    ...
    label: isRandomFive ? "5 Random Situations" : "Play All Situations",
  };

  localStorage.removeItem(PLAY_ALL_COMPLETE_KEY);
  setStoredJson(PLAY_ALL_KEY, session);
  window.location.href = `./softball-scenarios/${order[0]}/`;
});
