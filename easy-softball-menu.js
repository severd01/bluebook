...
function startSession(order, mode, label) {
  ...
  window.location.href = `./easy-softball-scenarios/${safeOrder[0]}/`;
}
...
function renderScenarioGrid() {
  ...
  scenarioGrid.innerHTML = scenarios
    .map(
      (scenario) => `
        <a class="scenario-link" href="./easy-softball-scenarios/${scenario.slug}/">
          <span class="scenario-number">${String(scenario.number).padStart(2, "0")}</span>
          <p class="scenario-title">${scenario.title}</p>
          <p class="scenario-meta">Open this easy-mode situation</p>
        </a>
      `
    )
    .join("");
  ...
  if (activeSession?.active) {
    ...
    if (resumeBtn) {
      ...
      resumeBtn.onclick = () => {
        ...
        window.location.href = `./easy-softball-scenarios/${slug}/`;
      };
    }
...
if (randomFiveBtn) {
  randomFiveBtn.addEventListener("click", () => {
    ...
    startSession(randomOrder, "random-5", "5 Easy Random Situations");
  });
}
