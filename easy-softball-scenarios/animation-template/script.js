...
function createScenarioTemplate(title, index) {
  return {
    title,
    system: "",
    description: "R1, ground ball to short.",
    runners: "R1",
    outs: "No Outs",
    ball: "Ground Ball",
    type: "dual-movement",
    ballFlightPath: "M 443 696 L 354 510",
    ...
    answerNotes: [
      {
        ...
        bullets: [
          "Trail the batter-runner no more than one-third of the way to first base in fair territory and read the play. As the lead runner approaches second base, move toward the holding zone halfway to third in foul ground, unless a play develops at third base. When a play develops at third base on the lead runner, move toward third base to first obtain the proper angle, then close your distance working to get an unobstructed view of the play and to obtain a final calling distance of 10-12 feet from the play. Remember, as the four elements come together; stop, read the play and make the call. Responsible for a subsequent throw on the lead runner (R1) at third base and any play at the plate.",
        ],
      },
      {
        heading: "B",
        bullets: [
          "Let the ball take you to the play, obtaining the best angle and distance possible. Responsible for any play at first base, second base and the last runner (batter-runner) to third base.",
        ],
      },
    ],
    paths: [
      {
        role: "P",
        startAnswer: { x: 443, y: 712 },
        routePoints: [
          { x: 443, y: 712 },
          { x: 430, y: 703 },
          { x: 456, y: 666 },
        ],
        waypoints: [{ x: 430, y: 703 }],
        endAnswer: { x: 456, y: 666 },
      },
      {
        role: "U1",
        startAnswer: { x: 542, y: 536 },
        routePoints: [
          { x: 542, y: 536 },
          { x: 558, y: 554 },
        ],
        waypoints: [],
        endAnswer: { x: 558, y: 554 },
      },
    ],
  };
}
...
scenarios[0] = {
  ...
  description: "Temporary sandbox for testing motion paths and copied coordinate animations.",
  runners: "Custom",
  outs: "N/A",
  ball: "Animation Test",
};
...
const initialScenarioIndex = currentScenarioSlug === "animation-template"
  ? 0
  : Math.max(DEFAULT_SCENARIO_ORDER.indexOf(currentScenarioSlug), 0);
...
function updatePathToolOutput() {
  ...
  pathToolStatusEl.textContent = `${capturePoints.length} point${capturePoints.length === 1 ? "" : "s"} captured.`;
}
