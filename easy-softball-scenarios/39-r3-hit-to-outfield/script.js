...
function createScenarioTemplate(title, index) {
  return {
    title,
    system: "",
    description: "R1 on 3rd, base hit to the outfield.",
    runners: "R1 on 3rd",
    outs: "No Outs",
    ball: "Base Hit",
    type: "dual-movement",
    ballFlightPath: "M 443 696 Q 347 369 254 288",
    ...
    answerNotes: [
      {
        ...
        bullets: [
          "Move out from behind the plate toward the holding zone halfway to third base in foul ground to an area where you can see all four elements and read the play.",
          "If there is no play developing at home, observe R1 touching home plate from the holding zone and be prepared to move back to the plate area when BU brings the batter-runner to third base.",
          "Responsible for any play on a lead runner (R1) at third base and any play at the plate.",
        ],
      },
      {
        heading: "B",
    ...
    paths: [
      {
        ...
        waypoints: [],
        endAnswer: { x: 363, y: 670 },
      },
      {
        role: "U1",
        startAnswer: { x: 369, y: 505 },
        routePoints: [
          { x: 369, y: 505 },
          { x: 473, y: 535 },
        ],
        waypoints: [],
        endAnswer: { x: 473, y: 535 },
      },
    ],
  };
}
